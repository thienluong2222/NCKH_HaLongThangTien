#!/usr/bin/env python3
"""
Đánh giá mô hình phân loại lễ hội với đo lường Inference Time
...
Inference Time: YOLO inference time và tổng thời gian phân loại per image
"""
...
class FestivalEvaluator:
    ...
    def __init__(self, model_path: str, csv_path: str, frame_dir: str, gemini_api_key: str = None):
        ...
        """
        print("=" * 60)
        print("🚀 KHỞI TẠO FESTIVAL EVALUATOR (với Inference Time)")
        print("=" * 60)
    ...
    def evaluate_single_sample(self, sample: dict, scenario: str = 'baseline') -> dict:
        """
        Đánh giá 1 sample, đo lường inference time
        ...
        Returns:
            dict: Kết quả đánh giá kèm yolo_inference_time và total_inference_time (giây)
        """
        image_path = sample['image_path']
        json_path = sample['json_path']
        ground_truth = sample['ground_truth']

        # Bắt đầu đo tổng thời gian phân loại
        total_start_time = time.perf_counter()

        # Step 1: YOLO Detection — đo riêng inference time của YOLO
        ...
        yolo_inference_time = yolo_end_time - yolo_start_time
        ...
        # Kết thúc đo tổng thời gian phân loại
    ...
    def calculate_metrics(self, results: list) -> dict:
        """
        Tính toán metrics từ kết quả đánh giá, bao gồm inference time stats
        ...
        yolo_inference_times = np.array([r['yolo_inference_time'] for r in results])
        total_pipeline_times = np.array([r['total_inference_time'] for r in results])

        inference_time_stats = {
            'yolo': {
                'mean_ms': float(np.mean(yolo_inference_times) * 1000),
                'std_ms': float(np.std(yolo_inference_times) * 1000),
                'min_ms': float(np.min(yolo_inference_times) * 1000),
                'max_ms': float(np.max(yolo_inference_times) * 1000),
                'median_ms': float(np.median(yolo_inference_times) * 1000),
            },
            'total_pipeline': {
                'mean_ms': float(np.mean(total_pipeline_times) * 1000),
                'std_ms': float(np.std(total_pipeline_times) * 1000),
                'min_ms': float(np.min(total_pipeline_times) * 1000),
                'max_ms': float(np.max(total_pipeline_times) * 1000),
                'median_ms': float(np.median(total_pipeline_times) * 1000),
            },
        }
    ...
    def _print_metrics(self, scenario: str, metrics: dict):
        ...
        print(f"   {'Metric':<30} {'YOLO':>12} {'Total Pipeline':>16}")
        print(f"   {'-' * 60}")
        print(f"   {'Mean (ms)':<30} {it['yolo']['mean_ms']:>12.2f} {it['total_pipeline']['mean_ms']:>16.2f}")
        print(f"   {'Std  (ms)':<30} {it['yolo']['std_ms']:>12.2f} {it['total_pipeline']['std_ms']:>16.2f}")
        print(f"   {'Min  (ms)':<30} {it['yolo']['min_ms']:>12.2f} {it['total_pipeline']['min_ms']:>16.2f}")
        print(f"   {'Max  (ms)':<30} {it['yolo']['max_ms']:>12.2f} {it['total_pipeline']['max_ms']:>16.2f}")
        print(f"   {'Median (ms)':<30} {it['yolo']['median_ms']:>12.2f} {it['total_pipeline']['median_ms']:>16.2f}")
    ...
    def _print_comparison_table(self, all_results: dict):
        ...
        metrics_to_compare = [
            ...
            ('YOLO Inference Mean (ms)', lambda m: f"{m['inference_time']['yolo']['mean_ms']:.2f}"),
            ('YOLO Inference Median (ms)', lambda m: f"{m['inference_time']['yolo']['median_ms']:.2f}"),
            ('Total Pipeline Mean (ms)', lambda m: f"{m['inference_time']['total_pipeline']['mean_ms']:.2f}"),
            ('Total Pipeline Median (ms)', lambda m: f"{m['inference_time']['total_pipeline']['median_ms']:.2f}"),
        ]
                ...
                'sample_results': [
                    {
                        ...
                        'yolo_inference_time_ms': r['yolo_inference_time'] * 1000,
                        'total_inference_time_ms': r['total_inference_time'] * 1000,
                    }
